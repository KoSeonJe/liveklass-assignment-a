...
- **과제 배경·목표**: LiveKlass BE 과제 A. 실시간 수강 신청 도메인에서 평가 핵심 3축(상태 전이 · 정원 관리 · 동시성 제어)을 깊이 있게 다루는 것을 목표로 함. 프로덕션 운영 부수 기능은 의도적으로 제외해 평가 핵심에 자원을 집중. 상세는 [§4 평가 핵심에 대한 해석](#평가-핵심에-대한-해석) · [§4 의도적으로 제외한 항목](#의도적으로-제외한-항목).
- **핵심 설계 한 줄 요약**: Facade(트랜잭션 외부, 인메모리 카운터·대기열·외부 결제 호출) ↔ Service(`@Transactional` 내부, DB 원자적 UPDATE) 2계층으로 책임 분리. 마지막 한 자리 동시성은 인메모리 `AtomicInteger` CAS + DB 조건부 `UPDATE`로 처리. 상세는 [§5-1](#5-1-동시성-제어--4안-비교-후-option-4-채택) · [§5-3](#5-3-트랜잭션-경계-facade--service-2-layer).
...
| 7 | 결제 실패 시 처리 | `Enrollment`를 `CONFIRMED → PENDING`으로 롤백 | 정원은 유지하되 재결제 시도 여지를 남김 |
| 8 | 대기열 저장 위치 | 인메모리 자료구조 (DB 미사용) | 단일 인스턴스 가정 하에 충분하며, DB 락 경합을 추가로 만들지 않기 위함 |
| 9 | 대기열 승급 트리거 | 수강 취소 / 결제 실패 보상 시점에 최선두 사용자 자동 승급 | 정원 반환 사건에 자연스럽게 결합 |
| 10 | 수강 신청 응답 코드 | 자리 확보 시 `201 Created`, 대기열 등록 시 `202 Accepted` | 리소스 생성 여부에 따라 시맨틱하게 구분 |
...
신청 요청
   │
   ▼
┌─ in-mem CAS 차감 ─┐── fail ─▶ 대기열 enqueue (202)
└────────┬──────────┘
         ▼
┌─ DB atomic UPDATE ┐── fail ─▶ 보상 (in-mem 원복 / 대기자 승급)
│ WHERE count<max   │
└────────┬──────────┘
         ▼
  Enrollment INSERT (PENDING) → 201
```
...
Enum의 `verifyTransitionTo()` + 도메인 메서드 두 곳에서 전이 강제.
...
| 전이 | 트리거 | 허용 | 비고 |
|---|---|---|---|
| (없음) → PENDING | 결제 진행 tx#1: Payment INSERT | ✅ | Payment row는 결제 시도 시점 생성 |
...
|---|---|---|
| 수강 신청 | seatCounter acquire / waitlist enqueue / 보상 release | Enrollment INSERT |
| 결제 | 외부 게이트웨이 charge() | Payment INSERT, Enrollment CONFIRMED |
| 취소 | 게이트웨이 cancel / seatCounter release / 대기열 승급 | Enrollment CANCELLED, Payment CANCELLED |
...
- `seatCounter.remaining == max_capacity - DB.current_count`
- 대기열 승급은 1건 이하
...
- 이유: ① 도메인이 단일 인스턴스·소규모로 cascade 자동화 이득 적음 ② JPA 연관관계 객체 그래프 로딩이 트랜잭션 경계·N+1 비용을 늘림 ③ Facade-Service 분리에서 Service는 ID 단위로만 다루는 게 단순.
...
- **버전 컬럼(`@Version`)**: 낙관적 락 미사용. 동시성 제어는 인메모리 CAS + DB 조건부 UPDATE로 일원화.
...
- **결제 보상 트랜잭션 자체 실패 / PG 취소 요청 실패에 대한 자동 재시도·DLQ 미구현** — CRITICAL 로그 + 수동 보정 의존.
